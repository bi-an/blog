...
> **单一事实来源（SSOT）**：本文件同时供人类作者与 AI 助手遵循。
> 新增/修改笔记、配图、配套代码前先读本文。AI 入口：`AGENTS.md`、`.cursor/rules/blog-content.mdc`。
> 本文件在仓库根目录，不在 Hexo `source/` 内。
...
- 目录/文件单复数命名规则：
  - `_posts` 下的目录统一采用单数名（类 Go），例如 `movie`。
  - Hexo 通用资源目录采用复数名，例如 `images`。
  - 笔记与资源文件名遵循下文「全量文件命名规范」。
---
