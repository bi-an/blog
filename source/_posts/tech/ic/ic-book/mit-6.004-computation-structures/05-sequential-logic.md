...
因此选用**宽容的**（lenient）MUX。在下列任一条件下，输入跳变后输出仍保持有效稳定：

1. 加载时 $G$ 为 HIGH：一旦 $D$ 有效稳定满 $t_{\mathrm{PD}}$，保证 $Q$ 与 $D$ 同值且稳定，与 $Q$ 初值无关。
2. 若 $Q$ 与 $D$ 均已有效稳定满 $t_{\mathrm{PD}}$，则此后 $G$ 上的跳变不影响 $Q$——这正是 $G$ 做 1→0 而不污染 $Q$ 所需。
3. 若 $G$ 为 LOW 且 $Q$ 已稳定至少 $t_{\mathrm{PD}}$，则此后 $D$ 上的跳变不影响输出。

宽容 alone 不够：还须保证信号在正确时刻稳定，才能用上上述性质。

## 8. 再加一点纪律（… With a Little Discipline）
...
1. $G$ 为 HIGH 时，把待存值放到 $D$；经 $t_{\mathrm{PD}}$ 后，$Q$ 上保证稳定有效（上节条件 1）。
2. 再等一个 $t_{\mathrm{PD}}$，让新 $Q$ 信息传遍锁存器内部。此时 $D$ **与**内部 $Q'$ 都已稳定至少 $t_{\mathrm{PD}}$（条件 2）。因此若 $D$ 稳定满 $2\,t_{\mathrm{PD}}$，$G$ 跳变不影响 $Q$。对 $D$ 的这一要求称为锁存器的**建立时间**（setup time）：$G$ 做 HIGH→LOW 之前，$D$ 必须已稳定有效多久。
3. 将 $G$ 置 LOW，仍保持 $D$ 稳定；再经 $t_{\mathrm{PD}}$ 让新 $G$ 传遍内部，满足条件 3，此后 $D$ 变化不影响 $Q$。

$G$ 跳变后 $D$ 还需保持稳定多久，称为**保持时间**（hold time）。建立时间与保持时间合称**动态纪律**（dynamic discipline）：$D$ 必须在 $G$ 跳变**前后**都稳定有效。遵守动态纪律，才能保证门控 HIGH→LOW 时可靠存下 $D$ 上的信息。
...
- 电路由边沿触发寄存器与组合逻辑“云”组成；寄存器输入到输出无组合通路，故整电路无组合环——从系统输入/寄存器输出到寄存器输入的路径不会两次经过同一组合块。
- 所有时钟器件共享**同一**周期时钟。多时钟域可行，但跨域时序很难分析，同钟更简单。
- 各数据信号何时变化大体不重要；关键是接到寄存器输入的信号稳定够久以满足建立时间，并保持够久以满足保持时间。

选时钟周期大于“寄存器输出 → 寄存器输入”每条路径的 $t_{\mathrm{PD}}$ 再加上建立时间，即可保证遵守动态纪律。额外好处：上升沿瞬间，电路别处几乎没有其它逻辑跳变，更新寄存器存储状态时噪声更少。
...
- $t_1$ = REG1 的 $t_{\mathrm{CD}}$ + 逻辑的 $t_{\mathrm{CD}}$：上升沿后 STAR 仍有效多久；须 $t_1 \ge$ REG2 的保持时间。
- $t_2$ = REG1 的 $t_{\mathrm{PD}}$ + 逻辑的 $t_{\mathrm{PD}}$ + REG2 的建立时间：下一上升沿最早可发生的时刻；须 $t_2 \le t_{\mathrm{CLK}}$（时钟周期）。过早则违反 REG2 动态纪律。
...
还须考虑**时钟偏斜**（clock skew）：时钟到达各寄存器时刻不同，效果上增大下游表观建立/保持时间（若无法预知偏斜符号）。

$t_{\mathrm{CLK}}$ 刻画性能。Intel 等按不同时钟频率卖处理器——芯片往往相同，工艺波动使部分芯片 $t_{\mathrm{PD}}$ 更好，可支持更小 $t_{\mathrm{CLK}}$、更高频率；测速后挑快的卖更高性能档。
